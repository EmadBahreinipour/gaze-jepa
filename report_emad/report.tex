...
the patched model reduces validation prediction MSE from $0.30$ to $0.046$
after five CPU epochs ($\sim$85\% relative drop). This does not claim a complete
...
The main result is the validation prediction MSE (Table~\ref{tab:loop:mse}).
Before training, the patched model gives a validation MSE of $0.2996$. After
five epochs, the matched validation pass gives $0.0462$. This is an $84.6\%$
reduction. The result means that the predictor learned to anticipate the target
fixation representation under the saliency-driven cropper.
...
\begin{table}[ht]
  ...
  \caption{Predictor mean-squared error against the EMA target
    ...
    All numbers are averaged across 24 validation frames from the six held-out
    validation videos, with each frame yielding four fixation transitions
    ($T{=}5$). The saliency source is Arash's trained
    \texttt{ResNetSaliency}. The last row re-loads the final checkpoint and
    replays the validation pass in matched conditions. The training-time number
    recorded at end of epoch~$5$ ($0.0447$) and the post-hoc reproduction
    ($0.0462$) agree closely despite stochastic fixation sampling.}
  \label{tab:loop:mse}
\end{table}
